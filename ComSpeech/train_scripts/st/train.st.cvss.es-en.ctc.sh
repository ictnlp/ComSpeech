exp=st.cvss.es-en.ctc
python fairseq/fairseq_cli/train.py data/comspeech/cvss_es_en/s2s \
    --user-dir ComSpeech \
    --st-data data/comspeech/cvss_es_en/speech2unigram --validate-task st --multiscale-modeling \
    --save-dir ComSpeech/checkpoints/$exp \
    --config-yaml config.yaml --multitask-config-yaml config_phoneme.yaml --train-subset train --valid-subset dev \
    ...
    --st-loss-weight 1.0 --w2p-dp-loss-weight 1.0 --label-smoothing 0.1 \
