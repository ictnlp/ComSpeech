...
class FastSpeech2StyleEncoder(FairseqEncoder):
    def __init__(self, args, src_dict, max_source_positions, embed_tokens=None):
        ...
        if embed_tokens is not None:
            ...
            self.embed_tokens = Embedding(
                len(src_dict), args.tts_input_embed_dim, padding_idx=self.padding_idx
            )
        self.input_proj = nn.Linear(args.tts_input_embed_dim, args.tts_encoder_embed_dim)
        self.embed_positions = PositionalEmbedding(
            max_source_positions, args.tts_encoder_embed_dim, self.padding_idx
        )
    ...
    def forward_embedding(self, src_tokens, src_lengths=None):
        x = self.embed_tokens(src_tokens)
        enc_padding_mask = src_tokens.eq(self.padding_idx)
        return x, enc_padding_mask
        
    def forward_encoder(self, x, enc_padding_mask, return_all_hiddens=False):
        x = self.input_proj(x)
    ...
    def forward_variance_adaptor_and_decoder(
        self,
        x,
        enc_padding_mask,
        durations=None,
    ...
    def forward(
        ...
        **kwargs,
        ...
        x, enc_padding_mask = self.forward_embedding(src_tokens, src_lengths)
        x, _ = self.forward_encoder(x, enc_padding_mask)
        out = self.forward_variance_adaptor_and_decoder(x, enc_padding_mask, durations, pitches, energies)
        return out


@register_model("fastspeech2_style")
class FastSpeech2StyleModel(FairseqEncoderModel):
    """
    Implementation for https://arxiv.org/abs/2006.04558
    We add a style encoder to provide additional style information.
    """
    ...
    def build_model(cls, args, task):
        encoder = FastSpeech2StyleEncoder(args, task.src_dict, args.max_source_positions)
...
@register_model_architecture("fastspeech2_style", "fastspeech2_style")
